...
class ColorSchemeGenerator:
    ...
    def generate_industry_scheme(self, industry, scheme_name):
        ...
        if industry.lower() not in industry_colors:
            raise ValueError(
                f"Industry '{industry}' not supported. Available: {list(industry_colors.keys())}"
            )
    ...
    def preview_scheme(self, scheme_dict):
        ...
        color_properties = {
            k: v for k, v in scheme_dict.items() if k.endswith("_COLOR")
        }
        ...
        print(
            f"  SECTIONS (Headers):   {self.color_swatch(section_color)} {section_color}"
        )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Generate professional color schemes for resumes"
    )
    ...
    method_group.add_argument(
        "--triadic", metavar="HEX_COLOR", help="Generate triadic scheme from base color"
    )
    method_group.add_argument(
        "--industry",
        choices=[
    ...
    # Options
    parser.add_argument("--name", required=True, help="Name for the color scheme")
    parser.add_argument(
        "--output-dir",
    ...
    parser.add_argument(
        "--preview", action="store_true", help="Show color preview before saving"
    )
    ...
    try:
        if args.monochromatic:
            scheme_dict = generator.generate_monochromatic_scheme(
                args.monochromatic, args.name
            )
        elif args.complementary:
            scheme_dict = generator.generate_complementary_scheme(
                args.complementary, args.name
            )
        elif args.triadic:
        ...
        if scheme_dict:
            ...
            print(
                f"   python resume_manager.py --generate-data --color-scheme {args.name}"
            )
